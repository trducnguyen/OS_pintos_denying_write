...
#define MAX_ARGS 128  // Bạn có thể điều chỉnh nếu cần

char *argv[MAX_ARGS];  // Mảng con trỏ chứa từng token
int argc = 0;          // Số lượng đối số
...
process_execute (const char *file_name) 
{
  ...
  /* Make a copy of FILE_NAME to pass to new thread. */
  fn_copy = palloc_get_page (0);
  if (fn_copy == NULL)
    return TID_ERROR;
  strlcpy (fn_copy, file_name, PGSIZE);

  /* Extract program name only for thread name. */
  char prog_name[NAME_MAX];
  strlcpy(prog_name, file_name, NAME_MAX);
  char *save_ptr;
  char *token = strtok_r(prog_name, " ", &save_ptr);

  /* Create a new thread using the program name, pass full file_name */
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  // ============================================================================
  // 💡 Tách file_name thành prog_name để load ELF, nhưng giữ nguyên file_name cho stack
  ...
  for (i = 0; i < ehdr.e_phnum; i++) 
    {
      ...
      switch (phdr.p_type) 
        ...
        case PT_NULL: case PT_NOTE: case PT_PHDR: case PT_STACK: default:
          break;
  ...
  /* Set up stack with argument passing */
  if (!setup_stack (esp, file_name))  // dùng original file_name để truyền argv
    goto done;
  ...
  if (kpage != NULL) 
  {
    ...
    if (success) 
    {
      *esp = PHYS_BASE;
      ...
      // Push arguments (strings) to stack
